...
//	 RESULTMSG *p_outbuf=p_arg->p_outbuf;
	...
	int result[600]={0};
	Uint16 nboxes = ArithDetect(pCfgs, pInFrameBuf, pInuBuf, pInvBuf, nWidth, nHeight, result);
...
//        std::cout<<outBuf->DetectRegionVehiSum[i]<<endl;
	...
	outBuf->udetPersonNum =pCfgs->ResultMsg.uResultInfo.udetPersonNum;
	for(int i = 0; i< pCfgs->ResultMsg.uResultInfo.udetPersonNum; i++)
	{
		outBuf->udetPersonBox[i].x  = pCfgs->ResultMsg.uResultInfo.udetPersonBox[i].x;
		outBuf->udetPersonBox[i].y = pCfgs->ResultMsg.uResultInfo.udetPersonBox[i].y;
		outBuf->udetPersonBox[i].width = pCfgs->ResultMsg.uResultInfo.udetPersonBox[i].width;
		outBuf->udetPersonBox[i].height  = pCfgs->ResultMsg.uResultInfo.udetPersonBox[i].height;
	}
