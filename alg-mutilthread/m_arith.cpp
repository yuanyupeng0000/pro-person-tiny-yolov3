...
int NCS_INDEX[MAX_NCS_NUM] = {-1};//计算棒是否在运行
int NCS_NUM = 0;//可用的计算棒数目
//char LABELS[][50] = {"background", "bus", "car", "truck", "bicycle", "motorbike", "person"};//检测类名
...
void py_init()
{
	...
	PyRun_SimpleString("import sys");
        printf("import sys ok\n");
	PyRun_SimpleString("sys.path.append('./')");
        printf("sys.path.append ok\n");
	if ( !pModule )
	{
                printf("PyImport_ImportModule mvnc-yolo-tiny...\n");
		pModule = PyImport_ImportModule("mvnc-yolo-tiny");
                printf("PyImport_ImportModule mvnc-yolo-tiny ok\n");
		if ( !pModule ) 
		{
...
int alg_mem_malloc(m_args *p)
{
	...
	if(initflag == 0)
	{
		...
		//p->pCfgs->g_graph = PyObject_CallObject(pFunc, NULL);////初始化计算棒
		result = PyObject_CallObject(pFunc, NULL);////初始化计算棒
		...
		{
			printf("NCS devices has %d\n",NCS_NUM);
		}
		Py_DECREF(result);
		Py_DECREF(pFunc);
		PyGILState_Release(gstate);
		memset(NCS_INDEX, -1, MAX_NCS_NUM * sizeof(int));
	}
	initflag++;
	//给相机分配计算棒
	printf("NCS_INDEX = [%d,%d,%d,%d,%d,%d]\n",NCS_INDEX[0],NCS_INDEX[1],NCS_INDEX[2],NCS_INDEX[3],NCS_INDEX[4],NCS_INDEX[5]);
	if(p->pCfgs->NCS_ID < 0)
	{
		for(i = 0; i < NCS_NUM; i++)
		{
...
int alg_mem_free(m_args *arg_arg)
{
	...
	//py_free();
	//相应的计算棒置0
	NCS_INDEX[arg_arg->pCfgs->NCS_ID] = -1;
	if (arg_arg->pCfgs) {
		...
		//if(arg_arg->pCfgs->g_graph)
		//	Py_DECREF(arg_arg->pCfgs->g_graph);
		free(arg_arg->pCfgs);
		arg_arg->pCfgs = NULL;
	}
	if ( arg_arg->pParams) {
...
//		printf("@@@@@@@@@@a@@@@@@@@@@@\n");fflush(NULL);
	...
	///////////////////////////////////////////////////////////根据标定区域设置行人检测框
	/*detMinX = min(pCfgs->calibration_point[0][0], pCfgs->calibration_point[1][0]);
	detMinX = min(detMinX, pCfgs->calibration_point[2][0]);
	detMinX = min(detMinX, pCfgs->calibration_point[3][0]);

	detMaxX = max(pCfgs->calibration_point[0][0], pCfgs->calibration_point[1][0]);
	detMaxX = max(detMaxX, pCfgs->calibration_point[2][0]);
	detMaxX = max(detMaxX, pCfgs->calibration_point[3][0]);

	detMinY = min(pCfgs->calibration_point[0][1], pCfgs->calibration_point[1][1]);
	detMinY = min(detMinY, pCfgs->calibration_point[2][1]);
	detMinY = min(detMinY, pCfgs->calibration_point[3][1]);

	detMaxY = max(pCfgs->calibration_point[0][1], pCfgs->calibration_point[1][1]);
	detMaxY = max(detMaxY, pCfgs->calibration_point[2][1]);
	detMaxY = max(detMaxY, pCfgs->calibration_point[3][1]);

	pCfgs->detectROI.x = detMinX;
	pCfgs->detectROI.width = detMaxX -detMinX + 1;
	pCfgs->detectROI.y = detMinY;
	pCfgs->detectROI.height = detMaxY - detMinY + 1;*/
	////////////////////////////////////////////////////detect params初始化参数
	pCfgs->target_id = 1;
...
Uint16 ArithDetect(ALGCFGS* pCfgs, unsigned char* pInFrameBuf, unsigned char* pInuBuf, unsigned char* pInvBuf, int width, int height, int* rst)
{
    ...
    //PyTuple_SetItem(ArgList, 3, pCfgs->g_graph);
	...
	Pyresult = PyObject_CallObject(pFunc, ArgList);//调用函数，完成传递
    PyObject* ret_objs;
...
inline Uint16 CalTargetSpeed(int idx, int laneID, ALGCFGS *pCfgs)//计算目标实际速度
{
			...
			for( j = 0; j < pCfgs->detClasses[i].classes_num; j++)
			{
				...
				if( strcmp(LABELS[i], "person") == 0)//6代表person
				{
					...
					for(k = 0; k < pCfgs->uDetectRegionNum; k++)
					{
						...
						/*overlap_x = min(pCfgs->detClasses[i].box[j].x + pCfgs->detClasses[i].box[j].width, pCfgs->detectROI.x + pCfgs->detectROI.width) - max(pCfgs->detClasses[i].box[j].x, pCfgs->detectROI.x);
						overlap_y = min(pCfgs->detClasses[i].box[j].y + pCfgs->detClasses[i].box[j].height, pCfgs->detectROI.y + pCfgs->detectROI.height) - max(pCfgs->detClasses[i].box[j].y, pCfgs->detectROI.y);
