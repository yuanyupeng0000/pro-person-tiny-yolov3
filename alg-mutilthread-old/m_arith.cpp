...
//char LABELS[MAX_CLASSES][50] = {"background", "bus", "car", "truck", "bicycle", "person", "motorbike"};
char LABELS[MAX_CLASSES][50] = {"bus", "car", "truck", "motorbike", "bicycle",  "person"};
int init_numpy()
{
	import_array();
	return 1;
...
#ifdef SAVE_VIDEO
        cv::rectangle(img, cv::Rect(pCfgs->detClasses[class_id].box[classes_num[class_id]].x,pCfgs->detClasses[class_id].box[classes_num[class_id]].y,pCfgs->detClasses[class_id].box[classes_num[class_id]].width,pCfgs->detClasses[class_id].box[classes_num[class_id]].height), cv::Scalar(0, 0 ,255), 1, 8, 0 );
#endif
